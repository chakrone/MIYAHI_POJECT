  ...
  return (
    <div className="card">
      <div className="card__header">
        ...
        <div className="card__icon icon-amber"><Receipt size={18} /></div>
      </div>
      {loading || !bill ? (
        ...
        <>
          ...
          <div className="billing-currency">{bill.currency} &middot; {bill.volume_m3} m&sup3;</div>
          {bill.breakdown?.map((t, i) => (
            <div className="billing-tier" key={i}>
              <span className="billing-tier__name">{t.tier}</span>
              <span style={{ color: 'var(--text-muted)', fontSize: 12 }}>{t.volume_m3}m&sup3; &times; {t.rate_per_m3}</span>
